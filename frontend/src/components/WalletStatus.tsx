...
const WalletStatus: React.FC = () => {
  const { connected, account, userTanks, loading, mintTank } = useWalletConnection()
    ...
    <div className="bg-game-primary p-4 rounded-lg">
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <p className="text-tank-green font-mono">{account?.address?.slice(0, 8)}...{account?.address?.slice(-6)}</p>
        </div>
        <div className="text-right">
          <p className="text-white text-sm">Số tank:</p>
          <p className="text-tank-green text-xl font-bold">{userTanks.length}</p>
        </div>
      </div>
